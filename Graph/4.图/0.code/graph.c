...
// 	for (int i = 0; i < graph->vexnum; i++) {
// 		flag[i] = 0;
// 		dist[i] = graph->A[start][i];
// 		path[i][0] = graph->V[start];
// 		path[i][1] = graph->V[i];
// 	}
// 	flag[0] = 1;
// 	int traversal = graph->vexnum;

// 	while (--traversal >= 0) {
// 		int currMin;
// 		int minDist = VERY_BIG;
// 		for (int i = 0; i < graph->vexnum; i++) {
...
// 		for (int i = 0; i < graph->vexnum; i++) {
// 			if (dist[currMin] + graph->A[currMin][i] < dist[i]) {
// 				dist[i] = dist[currMin] + graph->A[currMin][i];
// 				strcpy(path[i], path[currMin]);
// 				path[i][strlen(path[i])] = graph->V[i];
// 			}
// 		}
...
// 		printf("%c->%c", graph->V[start], graph->V[i]);
// 		printf("[%d]:", dist[i]);
// 		for (int j = 0; j < strlen(path[i]); j++) {
// 			// printf("strlen[path[%d]] = %ld  ", i, strlen(path[i]));
// 			if (j < strlen(path[i]) - 1) {
// 				printf("%c->", path[i][j]);
// 			} else {
// 				printf("%c\n", path[i][j]);
// 			}
